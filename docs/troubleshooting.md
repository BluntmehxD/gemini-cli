...
- **Q: How do I update BLNT to the latest version?**
  - A: If installed globally via npm, update BLNT using the command `npm install -g @google/blnt@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are BLNT configuration files stored?**
...
- **Error: Command not found (when attempting to run BLNT).**
  - **Cause:** BLNT is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure BLNT installation was successful.
